...
$URL = 'https://github.com/lainplus/rei'
$VERSION = '1.0.0'
$executors = []
$LOG = Logger.new(File.open(ENV['HOME'] + '/.rei_history', 'a+'))
...
def get_input_integer(min, max, options={})
  while true
    ...
    say "please enter an integer between #{ min } and #{ max }#{ options[:allow_no] ? ', or n for no' : '' }."
  end
end
...
def get_input_y_n
  while true
    ...
    say "please enter y for yes or n for no"
  end
end
  ...
  if User.has_command?('say')
    ...
    if Internet.connection_enable?
      ...
      if has_afplay || has_mpg123
         require 'open-uri'
         text = sanitize(text)
         speech_path = '/tmp/rei_speech.mp3'
...
# needs ReiConfig.get("web")!="false"
# edit ~/.reiconfig or say 'use web'
# example: "rei what is the weather"
def web_query(command)
  require 'net/http'
  encoded = URI.escape(command)
  chatmode = ReiConfig.get("chat")
  ...
  begin
    ...
    `open #{url}` if url and ReiConfig.get("web")
    `open #{image}` if image and ReiConfig.get("web") # or ascii-art :)
...
def main(commands)
  ...
  if responses.length == 1
    ...
    # edit ~/.reiconfig or say 'use web'
    if ReiConfig.get("web") && !command.empty? && !command.match("help")
      say web_query(command).sub("Jeannie",ReiConfig.get("name"))
    else
      help(command)
    end
  end
